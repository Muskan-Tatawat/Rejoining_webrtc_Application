...
const webSocket = new WebSocket("ws://192.168.43.75")
...
function handleSignallingData(data) {
 	switch (data.type) {
 		case "offer": //in receiver it is offer but in sender it is answer
 			peerconnection.setRemoteDescription(data.offer)

 			//once we get offer we will create answer using below function
 			createAndSendAnswer()
 			break
 		case "candidate":
...
//creating createAndSendAnswer()
function createAndSendAnswer(){
	//to create answer we have to make a call to createAnswer()
	//in the pear connection. this will return us a promise
	//with the answer in it 
	peerconnection.createAnswer((answer) => {
		 //once we have the answer we will set the local description
		 //and this will hold the answer
		 peerconnection.setLocalDescription(answer)

		 //now we will send this answer to the server by sendData()
		 //type will be send_answer and also attatch the answer
		 sendData({
		 	type: "send_answer",
		 	answer: answer
		 })
		 //we have second parameter which is error callback
		 //we will just log out this error
	},error => {
		console.log(error)
	})	
} 
...
// button
function joincall(){
	//once this function called we get the username from input box
	...
	navigator.getUserMedia({
		...
		//below this functionwe will send the user name of the person
		//we want to call.by sending this user name we also tell the
		//server we want to join the call to that person so that server
		//returns us their offer and their ice candidates .so we call
		//sendData() and type will be join_call.we dont need to attacth
		//user name cause it is automatically attatched in the sendData
		sendData({
			type: "join_call"
		})  
